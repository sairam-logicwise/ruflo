...
Then **Review #3's closure** (2026-09-23), after the whole 26-task implementation was reviewed for the first time as one body of work: 5 Critical, found by independent execution, not just reading. Confirmed every claim first (re-ran each exploit for real before touching code — the review's own numbers checked out exactly, down to the real corpus's 93.5%/6.5% input/output split and the real $0.20 total spend). Asked the user directly on C2's scope, since "build the MCP surface" and "restate the claim" are legitimate but wildly different-cost fixes — user chose to build it. Fixed all 5 Critical plus the 3 Important items the review itself flagged as blocking: a persisted, content-hash-pinned test-verification receipt closes the fake-`done` exploit (C1); a real `command-bridge.ts` adapter turns every existing `Command` into an MCP tool with zero logic duplicated, registering 20 tools across `record`/`run`/`phase-check`/`decompose`/`backfill` (C2, verified via `ruflo mcp exec` alone — no CLI shell-out); `quote`/`variance` both now require a required `actuals.source: 'proxy'|'measured'` field before stating a dollar figure or an unqualified hit rate, and `predict.ts` prices against the REAL input:output split instead of a hardcoded 1:3 guess that was silently 3.17x too expensive (C3/C4/C5, one shared fix); requirement creation can no longer self-service `accepted` (Important 1); a new `record task ready` command closes the `implementing -> verifying` hole the autonomy loop genuinely couldn't cross before (Important 6); a real active-voice false-positive regex bug is fixed with a genuine waiver mechanism (Important 14). Also fixed: the bookkeeping gap review #3 itself found (T11/T13/T14/T24 left unticked despite being committed) and a real cwd-dependent test flake (three different real `graphify-out/graph.json` files exist at three directory levels in this monorepo). Every fix independently re-verified by execution — reproducing the original exploit against a real hand-crafted repo, then confirming it closes — not just by reading the diff. The review file itself ([review-2026-09-23.md](./review-2026-09-23.md)) now carries inline "Closed" notes at every fixed item, honestly separating what changed from what was declared out of scope or genuinely still open (branch protection, `isLegalTransition` dead code, `contentHash` not covering frontmatter, and others — six Important items were never claimed fixed).

**Follow-up, same day: Important 5 closed too** — `contentHash` now hashes the whole canonicalized frontmatter, not just the body, closing the class C1 only patched one instance of. Extending its scope broke C1's own receipt check (a genuine receipt would read as stale on every real transition once frontmatter counted); fixed by excluding `verification` from the hash, same reasoning as `contentHash` excluding itself. Also caught and fixed two bugs surfaced while closing this: `auditDoneReceipt` never checked `verification.exitCode` (a forged "passing" receipt for a failed run went unnoticed), and `task-ready.ts`/`confirmRecord` never recomputed `contentHash` after mutating frontmatter. Verified end to end via the compiled CLI (a real `req -> confirm -> task -> run -> ready -> verify` cycle with a real `npm test` subprocess); all 37 real records migrated via `record validate --fix` in the same commit. Five Important items remain genuinely open now: branch protection, `isLegalTransition` dead code, and three others named in the review file.
