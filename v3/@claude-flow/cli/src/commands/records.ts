...
function kindDir(ctx: CommandContext, kind: RecordKind): string {
...
function findRecordPath(dir: string, id: string): string | undefined {
 ...
 */
function resolveBody(ctx: CommandContext, fallbackTitle: string): string {
...
function formatValidationError(error: Error): string {
...
const reqCommand: Command = {
  ...
  subcommands: [reqNewCommand, reqShowCommand, reqListCommand],
...
const taskNewCommand: Command = {
  ...
  options: [
    ...
    { name: 'status', description: 'backlog|active|blocked|done', type: 'string', default: 'backlog' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const claimed = claimAndWriteRecord(dir, RECORD_PREFIXES.task, slug, (id) => {
      const frontmatter = {
        id,
        title,
        status: (ctx.flags.status as string) ?? 'backlog',
